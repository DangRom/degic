...
BEGIN
    SELECT Id, Name, Activated, ClassType, Orders, ParrentId
    FROM Categorys
    ORDER BY Name, Id;
  END ;;
DELIMITER ;
...
BEGIN 
    SELECT Id, Name, Alias, Image, Status, Price, ShortDesciptions, Activated, Content, Orders
    FROM Courses
    ORDER BY Id;
  END ;;
DELIMITER ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `getAllParrentCategory`()
BEGIN
	SELECT Id, Name
	FROM Categorys
	WHERE ParrentId = 0
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `getAllParrentCategoryForCourse`()
BEGIN
	Select Id, Name 
	FROM Categorys
	WHERE ClassType = 1
...
BEGIN
    SELECT p.Id, p.Name, p.CategoryId, c.Name AS CategoryName, p.Activated, p.HomePage, p.Orders
    FROM Posts p
    INNER JOIN Categorys c on p.CategoryId = c.Id
    ORDER BY p.Orders, Name, Id;
  END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
BEGIN 
    SELECT Id, Name, Alias, Image, Activated, Descriptions, ClassType, Content, Orders, ParrentId
    FROM Categorys
    WHERE Id = pId
    LIMIT 1;
  END ;;
...
BEGIN 
    SELECT Name, Address, Phone, Hotline, Email, TaxCode, Google, Tweeter, Facebook, Description, About, WorkTime
    FROM Company
    LIMIT 1;
  END ;;
DELIMITER ;
...
BEGIN
    SELECT Address, Phone, Hotline, Email, Facebook, Google, Tweeter, TaxCode, WorkTime
    FROM Company
    LIMIT 1;
  END ;;
DELIMITER ;
...
BEGIN
    SELECT Facebook, Tweeter, Google, Hotline, WorkTime
    FROM Company
    LIMIT 1;
  END ;;
DELIMITER ;
...
BEGIN
    SELECT Id, Name, Alias, Image, Status, Price, ShortDesciptions, Activated, Content, Orders, CourseId
    FROM Courses
    WHERE Id = pId
    LIMIT 1;
  END ;;
...
BEGIN
    SELECT Name, Alias FROM Courses WHERE Activated = 1 ORDER BY Orders  desc LIMIT 5;
  END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
BEGIN
    SELECT Id, Name, Alias, ClassType, News, ParrentId
    FROM Categorys
    WHERE Activated = 1
    ORDER BY Orders;
  END ;;
...
BEGIN
    SELECT Id, Name, Alias, Image, ShortDescriptions, Activated, Content, CategoryId, HomePage, Orders
    FROM Posts
    WHERE Id = pId
    LIMIT 1;
  END ;;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertCategory`(IN pName TINYTEXT CHAR SET utf8, IN pAlias TINYTEXT CHAR SET utf8, IN pImage TINYTEXT CHAR SET utf8,
                                ...
                                IN pContent TEXT, IN pOrders INT, IN pParrentId INT)
BEGIN
    INSERT INTO Categorys (Name, Alias, Image, Activated, Descriptions, Content, ClassType, Orders, ParrentId)
    VALUES (pName, pAlias, pImage, pActivated, pDescriptions, pContent, pClassType, pOrders, pParrentId);
  END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
IN pContent LONGTEXT CHAR SET utf8, IN pPrice INT, IN pOrders TINYINT, IN pCourseId INT)
BEGIN
    INSERT INTO Courses (Name, Alias, Image, Status, ShortDesciptions, Content, Activated, Price, Orders, CourseId)
    VALUES (pName, pAlias, pImage, pStatus, pShortDescriptions, pContent, pActivated, pPrice, pOrders, pCourseId);
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
IN pImage TINYTEXT CHAR SET utf8, IN pShortDescriptions TINYTEXT CHAR SET utf8, IN pCategoryId INT, IN pOrders TINYINT,
IN pContent LONGTEXT  CHAR SET utf8, IN pActivated BIT, IN pHomePage BIT, IN pCreateDate DATETIME)
BEGIN 
    INSERT INTO Posts(Name, Alias, Image, ShortDescriptions, Content, Activated, CategoryId, CreateDate, HomePage, Orders)
      VALUES (pName, pAlias, pImage, pShortDescriptions, pContent, pActivated, pCategoryId, pCreateDate, pHomePage, pOrders);
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `updateCategory`(IN pId INT, IN pName TINYTEXT CHAR SET utf8, IN pAlias TINYTEXT CHAR SET utf8, IN pImage TINYTEXT CHAR SET utf8, IN pActivated TINYINT(1), IN pDescriptions TINYTEXT CHAR SET utf8, IN pClassType TINYINT(1), IN pContent TEXT, IN pOrders INT, IN pParrentId INT)
BEGIN
    UPDATE Categorys
    SET Name = pName, Alias = pAlias, Image = pImage, Activated = pActivated, Descriptions = pDescriptions,
        ClassType = pClassType, Content = pContent, Orders = pOrders, ParrentId = pParrentId
    WHERE Id = pId;
  END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
IN pWorkTime TINYTEXT CHAR SET utf8)
BEGIN 
    UPDATE Company
    SET Name = pName, Address = pAddress, Phone = pPhone, Hotline = pHotline, Email = pEmail,
        ...
        Description = pDescription, About = pAbout, WorkTime = pWorkTime;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `updateCourse`(IN pId INT, IN pName TINYTEXT CHAR SET utf8, IN pAlias TINYTEXT  CHAR SET utf8, IN pImage TINYTEXT  CHAR SET utf8,IN pStatus TINYTEXT CHAR SET utf8, IN pShortDescriptions TINYTEXT CHAR SET utf8, IN pActivated TINYINT(1),
IN pContent LONGTEXT CHAR SET utf8, IN pPrice INT, pOrders TINYINT, IN pCourseId INT)
BEGIN
    UPDATE Courses
    SET Name = pName, Alias = pAlias, Image = pImage, Status = pStatus, ShortDesciptions = pShortDescriptions,
        Activated = pActivated, Content = pContent, Price = pPrice, Orders = pOrders, CourseId = pCourseId
    WHERE Id = pId;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
BEGIN 
    UPDATE Posts
      SET Name = pName, Alias = pAlias, Image = pImage, ShortDescriptions = pShortDescriptions, Orders = pOrders,
...
-- Dump completed on 2017-08-15  1:43:06
